...
(function (eCrypt) {
    ...
    function eCryptInit(settings) {
            ...
            if (!(window.ewayPNVars)) {
                window.ewayPNVars = {
                    endpoints: {
                        ...
                        "Y": "http://dev.secure.ewaypayments.com/sharedpage",
        ...
        function handleAmexECMessage(event) {
            var result = "";
            var err = "";
            if ((event.origin.match(/ewaypayments.com/) || event.origin.match(/localhost:44300/)) && !event.data.match(/error_code/g) && event.data.match(/amexec/g) && event.data.length > 6) {
                setVar("FrameLoad", "ok");
                result = "Complete";
                err = "";
            } else {
        ...
        function handlePayNowMessage(event) {
            ...
            if (tag) {
                ...
                if (!(event.data.match(/^LoadOK/i))) {
                    ...
                    if (event.origin.match(/ewaypayments.com/) || event.origin.match(/localhost:44300/)) {
            ...
            if (event.origin.match(/ewaypayments.com/) || event.origin.match(/localhost:44300/)) {
                ...
                if (event.data.match(/^LoadOK/i)) {
                    ...
                    if (event.data == "Cancel") {
                        ...
                        tranid = event.data.replace("ID:", "").replace(/,AC:.*/, "");
                    }
                    if (modalCallback) {
    ...
    function RSAEncrypt(text) {
        ...
        var h = c.toString(16);
        if ((h.length & 1) == 0) return hex2b64(h);
        else return hex2b64("0" + h);
    }
...
const encrypt = (val, key) => {
    eCrypt.init();
    return eCrypt.encryptValue(val, key);
}
